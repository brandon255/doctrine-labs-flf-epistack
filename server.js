...
// Eval Contamination snapshot lives under data/eval-contamination/blocks/<cycle>/<company>/.
// Generated by data/eval-contamination/scripts/snapshot.js from the sibling
// doctrine-labs-eval-contamination-toolkit repo.
const EVAL_CONT_DIR = join(ROOT, "data", "eval-contamination");
// Brain OS snapshot lives under data/brain-os/. Generated by
// data/brain-os/scripts/snapshot.js from the sibling doctrine-labs-brain-os repo.
const BRAIN_OS_DIR = join(ROOT, "data", "brain-os");
...
export async function routeRequest({ method, pathname, search, body = {} }) {
  ...
  // ── Eval Contamination & Reviewer Independence Toolkit v2 ──
  // Live endpoints that serve the contamination audit + reviewer network +
  // cross-cycle delta. Same pattern as AISI but for the v2 toolkit.

  // Both cycles of companies (Winter 2025 had 8, Summer 2026 has 9)
  if (method === "GET" && pathname === "/api/eval/cycles") {
    const cycles = [
      { id: "winter-2025", label: "Winter 2025", companies: 8, indicators: 35, reviewers: 8, cutoff: "2025-11-08" },
      { id: "summer-2026", label: "Summer 2026", companies: 9, indicators: 37, reviewers: 7, cutoff: "2026-07-01" },
    ];
    return json(200, { ok: true, cycles });
  }

  // Per-company lineage, both cycles
  if (method === "GET" && pathname === "/api/eval/companies") {
    if (!existsSync(EVAL_CONT_DIR)) return json(503, { ok: false, error: "Eval Contamination snapshot not generated. Run: node data/eval-contamination/scripts/snapshot.js" });
    const cycles = ["winter-2025", "summer-2026"];
    const out = [];
    for (const cycle of cycles) {
      const cycleDir = join(EVAL_CONT_DIR, "blocks", cycle);
      if (!existsSync(cycleDir)) continue;
      for (const id of readdirSync(cycleDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name).sort()) {
        const file = join(cycleDir, id, "evidence_blocks.json");
        if (!existsSync(file)) continue;
        try {
          const raw = JSON.parse(readFileSync(file, "utf8"));
          const arr = Array.isArray(raw) ? raw : (raw.blocks || []);
          const registryFile = join(cycleDir, id, "source_registry.json");
          let has_registry = false;
          if (existsSync(registryFile)) {
            const reg = JSON.parse(readFileSync(registryFile, "utf8"));
            has_registry = Boolean(reg.lineages && Object.keys(reg.lineages).length > 0);
          }
          out.push({
            id,
            cycle,
            title: id.replace(/-/g, " "),
            block_count: arr.length,
            has_registry,
          });
        } catch { /* skip */ }
      }
  ...
  // Per-company detail
  const evalMatch = pathname.match(/^\/api\/eval\/company\/([a-z0-9_-]+)\/([a-z0-9_-]+)$/);
  if (method === "GET" && evalMatch) {
    const [, cycle, id] = evalMatch;
    const caseDir = join(EVAL_CONT_DIR, "blocks", cycle, id);
    const blocksFile = join(caseDir, "evidence_blocks.json");
    const registryFile = join(caseDir, "source_registry.json");
    if (!existsSync(blocksFile)) return json(404, { error: `unknown company: ${id} (cycle: ${cycle})` });
    try {
      const raw = JSON.parse(readFileSync(blocksFile, "utf8"));
      const arr = Array.isArray(raw) ? raw : (raw.blocks || []);
      const registry = existsSync(registryFile) ? JSON.parse(readFileSync(registryFile, "utf8")) : null;
      const lineages = registry && registry.lineages ? Object.keys(registry.lineages) : [];
      return json(200, {
        ok: true,
        company: id,
        cycle,
        ...
        lineage_labels: lineages.map((k) => registry.lineages[k].label || k),
        blocks: arr.slice(0, 30), // Cap response payload
      });
    } catch (err) {
  ...
  // Reviewer panel for a cycle
  const panelMatch = pathname.match(/^\/api\/eval\/panel\/([a-z0-9_-]+)$/);
  if (method === "GET" && panelMatch) {
    const cycle = panelMatch[1];
    const panelFile = join(EVAL_CONT_DIR, "blocks", "reviewers", `${cycle}-panel.json`);
    if (!existsSync(panelFile)) return json(404, { error: `no panel for cycle: ${cycle}` });
    try {
      const panel = JSON.parse(readFileSync(panelFile, "utf8"));
      // Compute edges + density server-side
      const reviewers = panel.reviewers || [];
      const edges = [];
      for (let i = 0; i < reviewers.length; i++) {
        for (let j = i + 1; j < reviewers.length; j++) {
          const shared = (reviewers[i].domain_assignments || []).filter(
            (d) => (reviewers[j].domain_assignments || []).includes(d),
          );
          if (shared.length > 0) {
            edges.push({ a: reviewers[i].id, b: reviewers[j].id, shared_domains: shared, weight: shared.length });
          }
        }
      }
      const maxEdges = (reviewers.length * (reviewers.length - 1)) / 2;
      const density = maxEdges > 0 ? edges.length / maxEdges : 0;
      // Domain counts
      const domainCounts = {};
      for (const r of reviewers) {
        for (const d of (r.domain_assignments || [])) {
  ...
  // Contamination audit (pre-computed)
  if (method === "GET" && pathname === "/api/eval/contamination") {
    const contamFile = join(EVAL_CONT_DIR, "contamination", "benchmark-shared-roots.json");
    if (!existsSync(contamFile)) return json(503, { ok: false, error: "Contamination audit not generated. Run: node data/eval-contamination/scripts/snapshot.js" });
    try {
      const data = JSON.parse(readFileSync(contamFile, "utf8"));
  ...
  // Cross-cycle delta (re-computed server-side from snapshots)
  if (method === "GET" && pathname === "/api/eval/cross-cycle") {
    if (!existsSync(EVAL_CONT_DIR)) return json(503, { ok: false, error: "Eval Contamination snapshot not generated." });
    const COMPANIES = [
    ...
    for (const company of COMPANIES) {
      const winterFile = join(EVAL_CONT_DIR, "blocks", "winter-2025", company, "evidence_blocks.json");
      const summerFile = join(EVAL_CONT_DIR, "blocks", "summer-2026", company, "evidence_blocks.json");
      const winterBlocks = existsSync(winterFile) ? JSON.parse(readFileSync(winterFile, "utf8")) : null;
      const summerBlocks = existsSync(summerFile) ? JSON.parse(readFileSync(summerFile, "utf8")) : null;
      const norm = (b) => Array.isArray(b) ? b : (b?.blocks || []);
      const extractUrl = (b) => {
        const ctx = String(b?.provenance?.context || "");
        const m = ctx.match(/https?:\/\/\S+/);
        return m ? m[0].replace(/[).,;]+$/, "") : null;
      };
      const winter = winterBlocks ? norm(winterBlocks) : null;
      const summer = summerBlocks ? norm(summerBlocks) : null;
      const winterUrls = winter ? new Set(winter.map(extractUrl).filter(Boolean)) : new Set();
      const summerUrls = summer ? new Set(summer.map(extractUrl).filter(Boolean)) : new Set();
      const overlap = summer ? [...summerUrls].filter((u) => winterUrls.has(u)).length : 0;
      const overlap_pct = summerUrls.size > 0 ? (overlap / summerUrls.size * 100).toFixed(1) : "0";
      rows.push({
        company,
        winter_blocks: winter?.length || 0,
        summer_blocks: summer?.length || 0,
        blocks_delta: (summer?.length || 0) - (winter?.length || 0),
        winter_urls: winterUrls.size,
        summer_urls: summerUrls.size,
        url_overlap: overlap,
        url_overlap_pct: overlap_pct,
      });
    }
  ...
  if (method === "GET" && pathname === "/api/eval/report") {
    const reportFile = join(EVAL_CONT_DIR, "EVAL_CONTAMINATION_REPORT.md");
    if (!existsSync(reportFile)) return json(404, { error: "Report not generated yet. Run snapshot.js then eval-report.js in the eval-contamination toolkit." });
    const text = readFileSync(reportFile, "utf8");
    return { status: 200, headers: { "content-type": "text/markdown; charset=utf-8" }, body: text };
  }

  // ── Brain OS endpoints ──
  if (method === "GET" && pathname === "/api/brain/status") {
    const topicsFile = join(BRAIN_OS_DIR, "topics-snapshot.json");
    const agentsFile = join(BRAIN_OS_DIR, "agents-snapshot.json");
    if (!existsSync(topicsFile)) return json(503, { ok: false, error: "Brain OS snapshot not generated yet. Run data/brain-os/scripts/snapshot.js" });
    const topicsData = JSON.parse(readFileSync(topicsFile, "utf8"));
    const agentsData = existsSync(agentsFile) ? JSON.parse(readFileSync(agentsFile, "utf8")) : {};
    const totalBlocks = topicsData.topics.reduce((a, t) => a + t.block_count, 0);
    return json(200, {
      ok: true,
      total_blocks: totalBlocks,
      topics: topicsData.topics,
      agents: Object.keys(agentsData).map((ide) => ({ ide, latest: agentsData[ide].file })),
      generated_at: topicsData.generated_at,
    });
  }
  if (method === "GET" && pathname === "/api/brain/today") {
    // Find the most recent daily report in the snapshot
    if (!existsSync(BRAIN_OS_DIR)) return json(503, { ok: false, error: "Brain OS snapshot missing" });
    const files = readdirSync(BRAIN_OS_DIR).filter((f) => f.startsWith("daily-") && f.endsWith(".md")).sort();
    if (files.length === 0) return json(404, { ok: false, error: "No daily reports snapshotted yet" });
    const latest = files[files.length - 1];
    const text = readFileSync(join(BRAIN_OS_DIR, latest), "utf8");
    return { status: 200, headers: { "content-type": "text/markdown; charset=utf-8" }, body: text };
  }
